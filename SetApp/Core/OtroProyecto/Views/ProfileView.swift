...
struct ProfileView: View {
    ...
    var body: some View {
        ZStack {
            VStack {
                Text(userInfo.usuario.name)
                    .font(.title)
                Text(canDelete ? "DO YOU REALLY WANT TO DELETE?" : "You are logged in as \(userInfo.usuario.email). Deleting your account will delete all content and remove your information from the database.  You must first re-authenticate")
                HStack {
                    Button("Cancel") {
                        canDelete = false
...
struct ProfileView_Previews: PreviewProvider {
    static var previews: some View {
        ProfileView(canDelete: .constant(false)).environmentObject(InformacionUsuario())
    }
}
